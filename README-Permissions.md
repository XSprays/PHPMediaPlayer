...
# PHP, CSS, JS files should be readable but not writable by the world
...
### Read-only directories (no write needed)

| Path            | Purpose                                      |
|-----------------|----------------------------------------------|
| `assets/css/`   | Stylesheets (served to the browser)          |
| `assets/js/`    | JavaScript (served to the browser)           |
| `admin/`        | Admin PHP pages (read + execute only)        |
...
6. **Set passwords (important)**
   - Go to `admin/settings.php` (or via Admin → Settings).
...
| Admin login                | `admin/login.php`        | Uses `admin_password_hash`                        |
| Admin dashboard            | `admin/index.php`        | Admin home                                        |
| Settings                   | `admin/settings.php`     | Configuration panel                               |
